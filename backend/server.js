...
const modelPath = path.join(__dirname, 'yolov8s.onnx');
const nmsModelPath = path.join(__dirname, 'nms-yolov8.onnx');
...
async function loadModel() {
    try {
        ...
        yolov8Session = await ort.InferenceSession.create(modelPath);
 ...
 * @param {ort.InferenceSession} yolov8Session session untuk model YOLOv8
...
async function detectImage (
    ...
    try {
        ...
        // Pastikan tensor dibersihkan setelah selesai
        tensor.dispose();
        config.dispose();
      }
}
